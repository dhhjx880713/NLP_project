...
dataset_files = ['data/hindi_hatespeech.tsv', 'data/bengali_hatespeech.csv']
for i in range(len(dataset_files))
    filename = os.path.split(dataset_files[i])[-1]
    name = filename.split('_')[0]
    data = pd.read_csv(file)
    if name == "hindi":

        data = pd.read_csv(dataset_files[i], sep='\t')
    elif name == "bengali":

        data = pd.read_csv(dataset_files[i])
    ...
    # f = open('data/stopwords-hi.txt', 'rb')
    # f = open('data/stopwords-bn.txt', 'rb')
    ...
    def train(dataloader, model, optimizer, criterion):
        ...
        return model.input.weight.data, model.output.weight.data, total_loss
    ...
    for i in range(epochs):
        ...
        model.input.weight.data, model.output.weight.data, loss = train(dataloader, model, optimizer, criterion)
